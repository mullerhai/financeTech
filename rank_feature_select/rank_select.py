...
col_names=['x_rank_'+str(item) for item in range(1,len(train_num_rank.columns))]
col_names=['mbl_num']+col_names
train_num_rank.columns=col_names
test_num_rank.columns=col_names


data=train_data_out[['mbl_num','target']]
train=pd.merge(data,train_num_rank)
...
def pipeline(iteration,random_seed,gamma,max_depth,lambd,subsample,colsample_bytree,min_child_weight):
    ...
    with open('E:/geo/code/rank_feature_select/feature_score/feature_score_{0}.csv'.format(iteration),'w') as f:
